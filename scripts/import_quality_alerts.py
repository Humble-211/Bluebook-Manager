...
# ── Configuration ──
SCAN_DIR = r"S:\QCLAB\Bluebooks Related\Quality Alert\Old QA"
...
def parse_die_numbers(raw: str) -> list[str]:
    ...
    Examples:
        "15899-15901"       → ["15899", "15901"]
        "15899, 15901"      → ["15899", "15901"]
        "15899/15901"       → ["15899", "15901"]
        "15925b"            → ["15925"]
        "15899/15901, 1234" → ["15899", "15901", "1234"]
    """
    # Split on / , or -
    ...
    for part in parts:
        ...
        if not part or not any(ch.isdigit() for ch in part):
            continue  # skip words with no digits (e.g. "Colour")
